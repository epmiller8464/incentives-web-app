...
const transformSurveyQuestions = (session, questionMap) => {
  ...
  const { userId, sessionId, contractorId } = session

  const questionIndexes = Object.keys(questionMap)

  console.log(questionIndexes)

  let utility, income, contactInfo, address, newSystemTypes,
    existingSystemTypes, existingSystemAge, homeSqFt, customerPriorities
  for (const index in questionIndexes) {
    ...
    switch (index) {
      case  '0':
        //newSystemTypes
        newSystemTypes = [...questionResponse.checkedSystemTypes]
        break
      case  '1':
        //utility
        utility = {
